...
void Syntax_Analyzer::Rat23S() {
	...
	check_symbol("#", 1, line_number);
	Opt_Declaration_List();
	check_symbol("#", 1, line_number);
	Statement_List_Start(1);
	check_symbol("EOF", 1, line_number);
...
int Syntax_Analyzer::Function(int exit_on_error) {
	...
	// the check_symbol function requires a line number in case it runs into
	// an error (so that it can print what line the error occured in).
	// it also passes by reference so that if a symbol is matched on another
	// line, the function calling check_symbol will have its line_number
	// variable updated. all other production rules that rely on check_symbol
	// need a line_number int variable.
	int line_number = get_line_number();
	
	// if check_symbol() didn't work, and exit_on_error is disabled,
	// return error code 1
	if (check_symbol("function", exit_on_error, line_number) != 0) {
		return 1;
	}
	...
	check_symbol("identifier", 1, line_number);
	check_symbol("(", 1, line_number);
	Opt_Parameter_List();
	check_symbol(")", 1, line_number);
	Opt_Declaration_List();
...
void Syntax_Analyzer::Parameter_List_Cont() {
	...
	if (check_symbol(",", 0, line_number) == 0) {
...
int Syntax_Analyzer::Qualifier(int exit_on_error) {
	Rule_list initial_list;
	copy_list(Productions, initial_list);
	int line_number = lexer->get_line_number();

	// Case 1: <Qualifier> -> int
	Productions.push_back("\t<Qualifier> -> int");

	if (check_symbol("int", 0, line_number) == 0) {
		return 0;
	}
	else {
		copy_list(initial_list, Productions);
	...
	if (check_symbol("bool", 0, line_number) == 0) {
		return 0;
	}
	else {
		copy_list(initial_list, Productions);
	...
	if (check_symbol("real", 0, line_number) == 0) {
		return 0;
	}
	else {
		copy_list(initial_list, Productions);
	...
	if (exit_on_error == 1) {
		...
		print_error("expected qualifier", line_number);
	}
	return 1;
}
...
int Syntax_Analyzer::IDs_Start(int exit_on_error) {
	...
	if (check_symbol("identifier", exit_on_error, line_number) != 0) {
		return 1;
	}
...
int Syntax_Analyzer::Statement(int exit_on_error) {
	...
	if (exit_on_error == 1) {
		...
		print_error("expected statement", line_number);
	}
	return 1;
}
...
int Syntax_Analyzer::Compound(int exit_on_error) {
	...
	if (check_symbol("{", exit_on_error, line_number) != 0) {
		return 1;
	}
...
int Syntax_Analyzer::If_Start(int exit_on_error) {
	...
	if (check_symbol("if", exit_on_error, line_number) != 0) {
		return 1;
	}
...
int Syntax_Analyzer::If_Cont(int exit_on_error) {
	...
	if (check_symbol("else", 0, line_number) == 0) {
		Statement(1);
		check_symbol("fi", 1, line_number);
		return 0;
	}
	else {
		copy_list(initial_list, Productions);
	...
	if (check_symbol("fi", 0, line_number) == 0) {
		return 0;
	}
	else {
		copy_list(initial_list, Productions);
	...
	if (exit_on_error == 1) {
		...
		print_error("expected 'fi' or 'else'", line_number);
	}
	return 1;
}
...
int Syntax_Analyzer::Return_Start(int exit_on_error) {
	...
	if (check_symbol("return", exit_on_error, line_number) != 0) {
		return 1;
	}
...
int Syntax_Analyzer::Return_Cont(int exit_on_error) {
	...
	if (exit_on_error == 1) {
		Productions.push_back("\t<Return Cont> -> <Expression Start> ; | ;");
		print_error("expected expression or ';'", line_number);
	}
	return 1;
}
...
int Syntax_Analyzer::Print(int exit_on_error) {
	...
	if (check_symbol("put", exit_on_error, line_number) != 0) {
		return 1;
	}
...
int Syntax_Analyzer::Scan(int exit_on_error) {
	...
	if (check_symbol("get", exit_on_error, line_number) != 0) {
		return 1;
	}
...
int Syntax_Analyzer::While(int exit_on_error) {
	...
	if (check_symbol("while", exit_on_error, line_number) != 0) {
		return 1;
	}
	...
	check_symbol(")", 1, line_number);
	Statement(1);
	check_symbol("endwhile", 1, line_number);
	return 0;
}
...
int Syntax_Analyzer::Relop(int exit_on_error) {
	...
	if (exit_on_error == 1) {
		Productions.push_back("\t<Relop> -> == | != | > | < | <= | =>");
		print_error("expected relational operator", line_number);
	}
	return 1;
}
...
int Syntax_Analyzer::Factor(int exit_on_error) {
	...
	if (exit_on_error == 1) {
		...
		print_error("expected primary", line_number);
	}
	return 1;
}
...
int Syntax_Analyzer::Primary_Start(int exit_on_error) {
	...
	if (check_symbol("identifier", 0, line_number) == 0) {
		Primary_Cont();
		return 0;
	}
	else {
	...
	if (check_symbol("integer", 0, line_number) == 0) {
		return 0;
	}
	else {
		copy_list(initial_list, Productions);
	...
	if (check_symbol("true", 0, line_number) == 0) {
		return 0;
	}
	else {
		copy_list(initial_list, Productions);
	...
	if (check_symbol("false", 0, line_number) == 0) {
		return 0;
	}
	else {
		copy_list(initial_list, Productions);
...
int Syntax_Analyzer::check_symbol(std::string symbol, int exit_on_error,
	...
	// otherwise, STAY on the same (current) token.
	...
	if (symbol == "identifier" || symbol == "integer") {
		if (current_token.first != symbol) {
			if (exit_on_error == 1) {  // doesn't match and exit_on_error on
				print_error("expected <" + symbol + ">", line_number);
			}
			else {
				return 1;
			}
			...
			Productions.clear();
			// if symbol appeared on new line, update line_number
			line_number = get_line_number();
			return 0;  // all other cases follow this same matching process
		}
	}
	// <Real>
	if (symbol == "<real>") {
		if (current_token.first != "real") {
			if (exit_on_error == 1) {
				print_error("expected <real>", line_number);
			}
			else {
				return 1;
			}
...
void Syntax_Analyzer::print_error(Message err_msg, int line_number) {
	*ofs << line_number << ": ERROR - " << err_msg << "\n";
	print_productions();
	*ofs << "\t";
	print_current_token();
	lexer->close_ifs();
